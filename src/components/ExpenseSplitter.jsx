...
export default function ExpenseSplitter({plan, rs, mc, c, lang}){
  ...
  return(<Card c={c}>
    ...
    {expenses.map(e=><div key={e.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:`1px solid ${c.BD}`,fontSize:'13px'}}>
      ...
      <div style={{display:'flex',gap:'8px',alignItems:'center'}}><span style={{color:c.T,fontWeight:'700'}}>{e.amt.toFixed(2)}€</span><button onClick={()=>rem(e.id)} title={t.removeExpense||'Remove expense'} style={{background:'none',border:'none',color:c.M2,cursor:'pointer',fontSize:'14px',padding:'0'}}>×</button></div>
    </div>)}
    {expenses.length>0&&<>
