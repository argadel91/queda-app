  ...
  return<React.Suspense fallback={<div style={{textAlign:'center',padding:'20px',color:c.M}}>...</div>}><>
    ...
    <div style={{display:'flex',gap:'8px',marginBottom:'8px'}}>
      ...
      <div style={{...cardSt,flex:1,marginBottom:0}}>
        ...
        {editingTime&&<ClockPicker autoOpen onClose={()=>setEditingTime(false)} value={planTime||''} onChange={async v=>{const up={...plan,startTimes:[v,...(plan.startTimes||[]).slice(1)],time:v};await updatePlan(up);setPlan(up);setEditingTime(false);}} c={c}/>}
