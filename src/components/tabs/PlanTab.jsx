...
export default function PlanTab(){
  ...
  const{plan,setPlan,isOrg}=planState;
  const{openSection,setOpenSection,setTab}=ui;
  const{mc,c,t,lang,planDate,planTime,planPlace,stop,tolerance,saveMyResp}=helpers;
  const minCap=stop.minAttendees||'';const maxCap=stop.maxCapacity||'';
    ...
    <div style={{background:c.CARD,border:`1px solid ${c.BD}`,borderRadius:'14px',padding:'14px',marginBottom:'10px'}}>
      <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:viewOpen?'12px':0}}>
        ...
        <div style={{display:'flex',gap:'4px',flexShrink:0}}>
          ...
          {isOrg&&<button aria-label="Edit stop" onClick={()=>setEditState('mode','stop_'+stop.id)} style={{background:'none',border:`1px solid ${c.BD}`,borderRadius:'8px',padding:'4px 8px',color:c.M2,cursor:'pointer',fontSize:'12px'}}>✏️</button>}
        </div>
      </div>
      ...
      {viewOpen&&<div className="fade-in">
        {/* Place */}
        {planPlace&&<div style={{background:myVote.placeOk===true?'#22c55e10':myVote.placeOk===false?'#ef444410':c.CARD2,border:`1px solid ${myVote.placeOk===true?'#22c55e40':myVote.placeOk===false?'#ef444440':c.BD}`,borderRadius:'10px',padding:'12px',marginBottom:'8px'}}>
          {planPlace.photo&&<img src={planPlace.photo} alt={planPlace.name||'Venue'} style={{width:'100%',height:'100px',objectFit:'cover',borderRadius:'8px',marginBottom:'8px'}}/>}
          ...
          {myVote.placeOk===false&&openSection._placeNoPopup&&<div className="fade-in" style={{marginTop:'8px',padding:'10px',background:'#ef444408',border:'1px solid #ef444420',borderRadius:'8px'}}>
        ...
        {/* Date + Time (invitee, only if place=Yes) */}
        {!isOrg&&myVote.placeOk===true&&<>
          ...
          {myVote.dateOk===true&&myVote.timeOk===true&&<>
              ...
              return<div style={{background:isOnTime?'#22c55e10':isLate?'#f59e0b10':c.CARD2,border:`1px solid ${isOnTime?'#22c55e40':isLate?'#f59e0b40':c.BD}`,borderRadius:'10px',padding:'10px',marginBottom:'8px'}}>
                ...
                {isLate&&<div style={{marginTop:'8px'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                    ...
                    <input type="number" inputMode="numeric" min="1" max={tolerance} value={myVote.lateMin} onChange={e=>setMyVote('lateMin',Math.min(Math.max(parseInt(e.target.value)||1,1),tolerance))} style={{width:'70px',background:c.CARD,border:`1px solid ${c.BD}`,borderRadius:'8px',padding:'8px',color:c.T,fontSize:'14px',fontFamily:'inherit',outline:'none',textAlign:'center'}}/>
            ...
            {stop.meetingPoint&&myVote.lateMin===0&&(()=>{
        ...
        {/* Organizer date/time info */}
        {isOrg&&<div style={{display:'flex',gap:'8px',marginBottom:'8px'}}>
        ...
        {/* Save (invitee) */}
        {!isOrg&&<div style={{marginTop:'10px'}}>
