...
import { Card, Lbl, Back } from '../components/ui.jsx'
...
export default function Discover({onBack,onJoin,c,lang,profile}){
  ...
  return(<div style={{padding:'24px',maxWidth:'420px',margin:'0 auto'}}>
    ...
    <div style={{display:'flex',gap:'6px',marginBottom:'12px',flexWrap:'wrap'}}>
      {modes.map(m=>(<button key={m.k} onClick={()=>setModeF(m.k)} style={{padding:'6px 12px',borderRadius:'20px',border:`1px solid ${modeF===m.k?c.A+'60':c.BD}`,background:modeF===m.k?`${c.A}15`:c.CARD,color:modeF===m.k?c.A:c.M2,fontSize:'12px',fontWeight:modeF===m.k?'700':'400',cursor:'pointer',fontFamily:'inherit'}}>{m.l}</button>))}
    </div>
